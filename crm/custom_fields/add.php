...
$values  = ['module'=>'contacts','field_name'=>'','label_el'=>'','label_en'=>'',
            'field_type'=>'text','options_json'=>'','is_required'=>0,'sort_order'=>0];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    checkCsrf();
    $values = [
        'module'      => in_array($_POST['module'] ?? '', $modules) ? $_POST['module'] : 'contacts',
    ...
    // Build options JSON for select type
    $optionsJson = null;
    ...
    if (empty($errors)) {
        try {
            $db->prepare(
                'INSERT INTO custom_fields (module, field_name, label_el, label_en, field_type, options_json, is_required, sort_order)
                 VALUES (?,?,?,?,?,?,?,?)'
            )->execute([
                $values['module'], $values['field_name'], $values['label_el'], $values['label_en'],
                $values['field_type'], $optionsJson, $values['is_required'], $values['sort_order'],
            ]);
            redirectWithFlash(APP_URL . '/custom_fields/index.php', 'success', t('success_add'));
        } catch (\PDOException $e) {
            if (str_contains($e->getMessage(), 'Duplicate entry')) {
                $errors[] = 'Field name "' . $values['field_name'] . '" already exists for this module.';
            } else {
                $errors[] = t('error_general');
            }
        }
    }
}
...
<div class="page-header">
    <h1><i class="bi bi-plus-square me-2 text-primary"></i><?= e(t('add_custom_field')) ?></h1>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                <input type="text" class="form-control font-monospace" name="field_name"
                       value="<?= e($values['field_name']) ?>"
                       placeholder="e.g. custom_date, priority" required
                       pattern="[a-z0-9_]+" title="Lowercase letters, numbers and underscores only">
                <div class="form-text">Lowercase, no spaces. e.g.: <code>my_field</code></div>
            </div>
            <div class="col-md-6">
            ...
            <div class="col-md-6">
                <label class="form-label"><?= e(t('field_type')) ?> <span class="text-danger">*</span></label>
            ...
            <div class="col-md-3 d-flex align-items-end pb-1">
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" name="is_required" id="is_required" <?= $values['is_required'] ? 'checked' : '' ?>>
                    <label class="form-check-label" for="is_required"><?= e(t('is_required')) ?></label>
                </div>
            </div>
            <div class="col-12" id="optionsRow" style="display:none;">
                <label class="form-label"><?= e(t('options')) ?> <span class="text-danger">*</span></label>
                <textarea class="form-control font-monospace" name="options_raw" rows="5"
                          placeholder="Option 1&#10;Option 2&#10;Option 3"><?= e($values['options_raw']) ?></textarea>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleOptions() {
        optRow.style.display = sel.value === 'select' ? '' : 'none';
    }
