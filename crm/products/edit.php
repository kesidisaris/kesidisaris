...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $db->prepare(
            'UPDATE products SET code=?, name_el=?, name_en=?, description_el=?, description_en=?,
             price=?, unit=?, vat_rate=?, active=?, updated_at=NOW() WHERE id=?'
        )->execute([
        ...
        saveCustomFieldValues('products', $id, $_POST);
        redirectWithFlash(APP_URL . '/products/view.php?id=' . $id, 'success', t('success_edit'));
    }
}
?>
...
<div class="page-header">
    <h1><i class="bi bi-pencil-square me-2 text-primary"></i><?= e(t('edit_product')) ?></h1>
    <a href="<?= APP_URL ?>/products/view.php?id=<?= $id ?>" class="btn btn-outline-secondary">
...
<form method="post" action="">
    ...
    <div class="form-section">
        ...
        <?= renderCustomFieldForm('products', $id) ?>
    </div>
